...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 w-full bg-white/80 backdrop-blur-md border-b border-gray-100 z-[9999]">
        ...
        <div className="md:hidden pointer-events-none">
          {menuOpen && (
            <motion.div
              ...
              className="bg-white border-t border-gray-100 shadow-lg absolute w-full pointer-events-auto"
            >
              {NAV_LINKS.map((item, i) => (
                <div key={i}>
                  {/* If item has NO children → make the entire row a Link */}
                  {!item.children ? (
                    <Link
                      href={item.href!}
                      onClick={() => setMenuOpen(false)}
                      className="block px-6 py-3 text-gray-700 hover:bg-blue-50 hover:text-blue-700 font-medium"
                    >
                      {item.label}
                    </Link>
                  ) : (
                    <>
                      {/* Parent row for submenu toggle */}
                      <div
